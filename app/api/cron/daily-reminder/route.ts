              ...
              <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:600px;background:#ffffff;border-radius:24px;box-shadow:0 10px 40px rgba(0,0,0,0.06);overflow:hidden;">
                <tr>
                  <td style="background:linear-gradient(135deg,#4F46E5,#7C3AED,#EC4899);padding:32px 30px;text-align:center;">
                    <h1 style="margin:0;font-size:32px;font-weight:700;color:#ffffff;letter-spacing:-0.5px;">✨ Note of Life</h1>
                    <p style="margin:6px 0 0 0;font-size:16px;color:rgba(255,255,255,0.85);">Every Memory Matters</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:40px 30px;background:#ffffff;">
                    <h2 style="margin:0 0 8px 0;font-size:22px;font-weight:600;color:#1F2937;">Hello ${name || "there"}! 👋</h2>
                    <p style="margin:0 0 20px 0;font-size:16px;line-height:1.6;color:#4B5563;">
                      It's a beautiful day to write. 🌿 Take a few minutes to reflect, capture your thoughts, and preserve today's memory.
                    </p>
                    ...
                    <div style="text-align:center;margin:32px 0;">
                      <a href="${appBaseUrl}/write" style="display:inline-block;background:linear-gradient(135deg,#4F46E5,#7C3AED);color:#ffffff;padding:14px 40px;border-radius:9999px;text-decoration:none;font-weight:600;font-size:16px;box-shadow:0 4px 14px rgba(79,70,229,0.35);">✍️ Write Now</a>
                    </div>
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const user of users) {
      try {
        const html = buildReminderEmail(user.name || "there", user.email);
        await sendEmail({
          to: user.email,
